...
class PrinterRenderer: UIPrintPageRenderer {
    ...
    private func drawLabel(from spec: [String: Any], forPageAt index: Int, in rect: CGRect) {
        ...
        if showIndex {
            let format = (label == nil || label!.isEmpty) ? "%ld" : label!
    ...
    private func pointFromPosition(
        ...
        if let top = top {
            ...
            y = rect.origin.y - size.height - CGFloat(PrinterUnit.convert(bottom))
        }

        if let left = left {
            x = rect.origin.x + CGFloat(PrinterUnit.convert(left))
        } else if let right = right {
            x = rect.size.width - size.width - CGFloat(PrinterUnit.convert(right))
        }
