...
class PrintManager(private val context: Context) {
    ...
    fun print(
        ...
        when (PrintContent.getContentType(content, context)) {
            PrintContent.ContentType.IMAGE -> printImage(content!!, settings, callback)
            PrintContent.ContentType.PDF -> printPdf(content!!, settings, callback)
        ...
        (context as Activity).runOnUiThread {
            ...
            view!!.webViewClient = object : WebViewClient() {
                ...
                override fun onPageFinished(webView: WebView, url: String) {
                    printWebView(this@PrintManager.view!!, settings, callback)
    ...
    private fun printPdf(path: String, settings: JSONObject, callback: (Boolean) -> Unit) {
        val stream = PrintContent.open(path, context) ?: return
    ...
    private fun printImage(path: String, settings: JSONObject, callback: (Boolean) -> Unit) {
        val bitmap = PrintContent.decode(path, context) ?: return
    ...
    private fun isPrintJobCompleted(jobName: String): Boolean {
        val job = findPrintJobByName(jobName)
        return job == null || job.info.state <= PrintJobInfo.STATE_COMPLETED
    }
